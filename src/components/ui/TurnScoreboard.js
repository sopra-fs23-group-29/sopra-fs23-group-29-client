...
export const TurnScoreboard = (props) => {
  ...
  const handleContinue = () => {
    webSocket.send(`/app/games/${params.id}/readyMovePlayers`, {
      userToken: userToken,
    });
  };
  ...
  return (
    <BaseContainer className="turn-scoreboard container">
      ...
      <div>
        <div
          className="turn-scoreboard table-row"
        ...
        {rankingQuestion.countryList.map((country, index) => {
          ...
          const player = scoreboardEntries.find(
            (p) => p.guessCountryCode == country.cioc
          );
          ...
          return (
            <div className="turn-scoreboard table-row" key={index}>
              ...
              <div
                style={{
                  backgroundColor: player ? player.playerColor : "transparent",
                  borderRadius: "50%",
                  width: "30px",
                  height: "30px",
                  color: "white",
              ...
              <div>
                {player
                  ? `${player.currentScore} ${
                      player.currentScore === 1 ? "Point" : "Points"
                    }`
                  : ""}
              </div>
              <div>{statistic}</div>
